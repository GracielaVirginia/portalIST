...
@section('admin')
    <div class="min-h-screen w-full relative overflow-hidden">
        ...
        <img src="{{ asset('images/bg-admin-light.jpg') }}" alt="Fondo administrador"
            class="absolute inset-0 w-full h-full object-cover opacity-90 dark:hidden pointer-events-none select-none" />
        <img src="{{ asset('images/bg-admin-dark.jpg') }}" alt="Fondo administrador oscuro"
        ...
        <div
            class="absolute inset-0 bg-gradient-to-br from-purple-900/50 via-purple-800/35 to-fuchsia-700/30 mix-blend-multiply">
        </div>
        ...
        <div class="absolute -top-40 -left-40 h-[420px] w-[420px] rounded-full blur-3xl bg-fuchsia-400/25 animate-pulse">
        </div>
        <div
            class="absolute -bottom-48 -right-48 h-[520px] w-[520px] rounded-full blur-3xl bg-purple-400/25 animate-[pulse_4s_ease-in-out_infinite]">
        </div>
        ...
        <div class="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-center">
                ...
                <section class="lg:col-span-7 space-y-6 text-white bg-purple-900 p-4 rounded-xl">
                    ...
                    <div class="flex items-center gap-3">
                        <div class="flex items-center gap-2">
                            ...
                            {{-- Etiqueta de entorno --}}
                            {{-- <span
                                class="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-white/15 border border-white/20">
                ...
                <section class="lg:col-span-5">
                    <div
                        class="w-full max-w-md ml-auto rounded-3xl border border-white/20 dark:border-white/10 bg-white/40 dark:bg-gray-700/40 backdrop-blur-md shadow-2xl">
                        <div class="p-6 sm:p-8 text-gray-900 dark:text-gray-100">
                            ...
                            <form method="POST" action="{{ route('admin.login.attemp') }}" class="mt-6 space-y-4"
                                novalidate>
                                @csrf
                                ...
                                <div>
                                    <label for="username"
                                        class="block text-sm font-semibold text-purple-900 dark:text-gray-200">
                                        Usuario o Email
                                    </label>
                                    <input id="username" name="username" type="text" placeholder="admin@ist.cl"
                            ...
                            {{-- Links secundarios --}}
                            {{-- Enlace que abre el modal (Admin → Galen) --}}
                            <div class="mt-4 text-center">
                                <a href="#"
                                    onclick="document.getElementById('adminGalenSupportModal').classList.remove('hidden'); document.getElementById('adminGalenSupportModal').classList.add('flex'); return false;"
                            ...
                            {{-- MODAL: Soporte a Galen --}}
                            <div id="adminGalenSupportModal"
                                class="fixed inset-0 z-50 hidden items-center justify-center bg-black/50 backdrop-blur-sm p-4">
                                <div
                                    class="w-full max-w-lg rounded-2xl bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 shadow-2xl border border-gray-200 dark:border-gray-700">
                                    <div
                                        class="flex items-center justify-between px-4 py-3 border-b border-gray-200 dark:border-gray-700">
                                        <h3 class="text-base font-semibold">Enviar ticket (Admin → Galen)</h3>
                                        <button
                                            class="rounded-lg px-2 py-1 text-sm text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800"
                                            onclick="document.getElementById('adminGalenSupportModal').classList.add('hidden'); document.getElementById('adminGalenSupportModal').classList.remove('flex');">
                                            Cerrar
                                        </button>
                                    </div>

                                    <form method="POST" action="{{ route('admin.support.galen.store') }}"
                                        ...
                                        <div>
                                            ...
                                            <input type="email" name="email" value="{{ old('email') }}" required
                                                class="w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100 px-3 py-2">
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <textarea name="detalle" rows="4" required
                                                class="w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100 px-3 py-2">{{ old('detalle') }}</textarea>
                                        </div>

                                        <div>
                                            <label class="block text-sm font-semibold mb-1">Adjunto (opcional)</label>
                                        ...
                                        <div class="flex justify-end gap-2 pt-1">
                                            <button type="button"
                                                class="rounded-lg px-4 py-2 text-sm border border-gray-300 dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-800"
                                                onclick="document.getElementById('adminGalenSupportModal').classList.add('hidden'); document.getElementById('adminGalenSupportModal').classList.remove('flex');">
                                                Cancelar
                                            </button>
                                            <button type="submit"
                                                class="rounded-lg px-4 py-2 text-sm font-semibold bg-purple-900 text-white hover:bg-purple-800">
                                                Enviar ticket
                                            </button>
                                        </div>
                                    </form>
                        ...
                        {{-- <div class="px-6 pb-6">
                            <div
                                class="mt-4 rounded-2xl border border-purple-200/60 dark:border-purple-800/60 bg-purple-50/70 dark:bg-purple-950/30 p-4">
                                <p class="text-xs leading-relaxed text-purple-900/90 dark:text-purple-100">
                                    Para mayor seguridad, activa 2FA en tu perfil y mantén tus credenciales en un gestor de
                                    contraseñas.
                                </p>
                            </div>
                        </div> --}}
