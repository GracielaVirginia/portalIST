...
<div class="px-6 py-6">
  <div class="flex items-center justify-between mb-4">
    ...
    <a href="{{ route('admin.users.unregistered') }}"
       class="inline-flex items-center gap-2 bg-purple-900 text-white text-sm font-semibold px-4 py-2 rounded-full shadow-sm hover:bg-purple-800 hover:shadow-md transition mt-12">
  ...
  <div class="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-6 shadow-sm">
    <form method="POST" action="{{ route('admin.users.store') }}" enctype="multipart/form-data" class="space-y-4">
      ...
      <details class="group rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden" open>
        <summary class="cursor-pointer bg-purple-50 dark:bg-purple-900/20 px-4 py-3 font-semibold text-purple-900 dark:text-purple-200 flex items-center justify-between">
          <span>1) Datos del paciente</span>
          <span class="text-sm text-purple-700/70 dark:text-purple-200/70 group-open:hidden">mostrar</span>
          <span class="text-sm text-purple-700/70 dark:text-purple-200/70 hidden group-open:inline">ocultar</span>
        </summary>
      ...
      <details class="group rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
        ...
        <div class="p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div class="md:col-span-2 flex flex-wrap items-center gap-6 mt-1">
      ...
      {{-- 4) Atención, Seguridad y Archivos --}}
      <details class="group rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
        <summary class="cursor-pointer bg-purple-50 dark:bg-purple-900/20 px-4 py-3 font-semibold text-purple-900 dark:text-purple-200 flex items-center justify-between">
          <span>4) Atención, seguridad y archivos</span>
          <span class="text-sm text-purple-700/70 dark:text-purple-200/70 group-open:hidden">mostrar</span>
          <span class="text-sm text-purple-700/70 dark:text-purple-200/70 hidden group-open:inline">ocultar</span>
        </summary>
        <div class="p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div class="md:col-span-2 grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <label class="block text-sm font-semibold text-purple-900 dark:text-purple-200 mb-1">
                PDF Receta (url_pdf_receta)
              </label>
              <input type="file" name="url_pdf_receta" accept="application/pdf"
                     class="w-full rounded-lg border px-3 py-2 focus:ring-2 focus:ring-purple-300 file:mr-4 file:py-2 file:px-3 file:rounded file:border-0 file:bg-purple-100 file:text-purple-700 hover:file:bg-purple-200">
            </div>
            <div>
              <label class="block text-sm font-semibold text-purple-900 dark:text-purple-200 mb-1">
                PDF Licencia (url_pdf_licencia)
              </label>
              <input type="file" name="url_pdf_licencia" accept="application/pdf"
                     class="w-full rounded-lg border px-3 py-2 focus:ring-2 focus:ring-purple-300 file:mr-4 file:py-2 file:px-3 file:rounded file:border-0 file:bg-purple-100 file:text-purple-700 hover:file:bg-purple-200">
            </div>
            <div>
              <label class="block text-sm font-semibold text-purple-900 dark:text-purple-200 mb-1">
                PDF Informe (url_pdf_informe)
              </label>
              <input type="file" name="url_pdf_informe" accept="application/pdf"
                     class="w-full rounded-lg border px-3 py-2 focus:ring-2 focus:ring-purple-300 file:mr-4 file:py-2 file:px-3 file:rounded file:border-0 file:bg-purple-100 file:text-purple-700 hover:file:bg-purple-200">
            </div>
          </div>
        </div>
