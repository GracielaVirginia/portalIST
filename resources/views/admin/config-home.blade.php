    ...
    <div class="max-w-5xl mx-auto px-6 py-8">
        <div class="flex items-center justify-between mb-4">
            <div class="flex items-center gap-3">
                <x-admin.nav-actions backHref="{{ route('admin.dashboard') }}" logoutRoute="admin.logout" variant="inline"
                    {{-- o "sticky" si la tabla es larga --}} />
            </div>
        </div>
        ...
        <form method="POST" action="{{ route('admin.config.home.update') }}" class="space-y-8">
            ...
            <div class="mt-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4 items-start">
                            ...
                            <div id="carouselImagenes"
                                ...
                                @foreach ($imagenes as $img)
                                    <div class="inline-block align-top mr-3 snap-start">
                                        <div
                                            class="relative group w-44 h-32 rounded-lg overflow-hidden border border-gray-300 dark:border-gray-700 bg-black/5">
                                            <img src="{{ asset('images/' . $img->nombre) }}" alt="{{ $img->nombre }}"
                                                class="w-44 h-32 object-cover pointer-events-none">
                                            {{-- Overlay con acciones --}}
                                            <div
                                                class="absolute inset-0 opacity-0 group-hover:opacity-100 transition flex flex-col items-center justify-center gap-2 bg-black/45">
                                                <button type="button"
                                                    class="px-3 py-1.5 rounded-md bg-purple-600 text-white text-xs font-semibold hover:bg-purple-500"
                                                    onclick="seleccionarImagen({{ $img->id }}, '{{ $img->nombre }}', '{{ asset('images/' . $img->nombre) }}', this)">
                                                    Seleccionar
                                                </button>
                                        ...
                                        <div class="mt-1 text-[11px] text-center text-gray-600 dark:text-gray-400 truncate w-44"
                                            title="{{ $img->nombre }}">
                                            {{ $img->nombre }}
                                        </div>
                                    </div>
                    ...
                    <div class="md:col-span-1">
                        <div
                            class="p-4 rounded-xl border border-purple-300/40 dark:border-purple-700/40 bg-purple-50/30 dark:bg-gray-800/40">
            ...
            <div>
                ...
                <select name="home_section_tipo" id="home_section_tipo"
                ...
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Selecciona qué tipo de bloque se mostrará en la
                    parte inferior del portal.</p>
            </div>
            ...
            <div id="bannerConfig" class="{{ $tipo === 'banner' ? '' : 'hidden' }}">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Texto del
                            botón</label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Enlace del
                            botón</label>
            ...
            <div id="cardsConfig" class="{{ $tipo === 'cards' ? '' : 'hidden' }}">
                ...
                <p class="text-sm text-gray-600 dark:text-gray-400 mb-3">Edita los textos o íconos que aparecerán en cada
                    card.</p>

                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    @foreach ($cards as $i => $c)
                        <div
                            class="p-4 rounded-xl border border-purple-200/50 dark:border-purple-700/50 bg-purple-50/30 dark:bg-gray-800/40">
                            <h3 class="font-semibold text-purple-900 dark:text-purple-100 mb-2">Card {{ $i + 1 }}
                            </h3>
<label class="block text-xs text-gray-600 dark:text-gray-400">Icono</label>
<select name="cards[{{ $i }}][icon]"
        class="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-800 text-sm px-3 py-2 mb-2">
  @php $icon = $c['icon'] ?? '🩺'; @endphp
  ...
  <option value="💬" {{ $icon==='💬' ? 'selected' : '' }}>💬</option>
    ...
    <script>
        (function() {
            ...
            btnPrev?.addEventListener('click', () => carrusel.scrollBy({
                left: -scrollItemWidth() * 2,
                behavior: 'smooth'
            }));
            btnNext?.addEventListener('click', () => carrusel.scrollBy({
                left: scrollItemWidth() * 2,
                behavior: 'smooth'
            }));
    ...
    <script>
        (function() {
            ...
            window.seleccionarImagen = async function(id, nombre, url, btnEl) {
                // UI inmediata
                if (inputSel) inputSel.value = nombre;
                if (preview) {
                    preview.src = url;
                    preview.classList.add('ring-4', 'ring-purple-500');
                    setTimeout(() => preview.classList.remove('ring-4', 'ring-purple-500'), 500);
                }

                // Persistir en BD
                const endpoint = @json(route('admin.images.select', ['image' => '__ID__'])).replace('__ID__', String(id));
                try {
                    ...
                    document.querySelectorAll('#carouselImagenes .group').forEach(g => g.classList.remove(
                        'ring-2', 'ring-purple-500'));
                    btnEl.closest('.group')?.classList.add('ring-2', 'ring-purple-500');
                } catch (e) {
                    console.error('No se pudo marcar como seleccionada:', e);
                    alert('No se pudo marcar esta imagen como seleccionada.');
                }
            };
        })();
    </script>
