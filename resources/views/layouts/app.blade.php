...
<head>
  ...
  {{-- Aplicar tema temprano: CLARO por defecto; DARK solo si localStorage.theme === "dark" --}}
  <script>
    (function () {
      try {
        const saved = localStorage.getItem('theme');
        const isDark = saved === 'dark';  // ✅ sin autodetección del sistema
...
<div class="min-h-screen flex flex-col" x-data="{ helpOpen:false, openManual:false, openVideo:false, openFaq:false }">
  ...
  <footer class="absolute z-20" id="draggableFooter" style="left: 20px; bottom: 20px;">
...
<script>
  function getCurrentDark() {
    // ✅ Solo preferencia explícita del usuario (claro por defecto)
  ...
  document.addEventListener('DOMContentLoaded', () => {
    const dark = getCurrentDark();   // ← ya no consulta el sistema
    applyDarkMode(dark);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const grip = document.getElementById('footerGrip') || footer;
